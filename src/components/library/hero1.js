...
const Hero1 = ({ heroData = defaultHeroData }) => {
  ...
  const handleCardMouseEnter = e => {
    Object.assign(e.currentTarget.style, heroStyles.darkCardHoverStyle)
  }

  const handleCardMouseLeave = e => {
    e.currentTarget.style.transform = 'translateY(0)'
    e.currentTarget.style.boxShadow = heroStyles.darkCardStyle.boxShadow
  }
        ...
        <Grid container spacing={3}>
          {cards.map((card, index) => (
            ...
            >
              ...
              >
                <Box
                  style={heroStyles.darkCardStyle}
                  className='brand-hover'
                  onMouseEnter={handleCardMouseEnter}
                  onMouseLeave={handleCardMouseLeave}
                >
                  <div style={heroStyles.iconContainerStyle}>
                    <div style={heroStyles.darkIconStyle}>{iconMap[index]}</div>
                  </div>
                  <h3 style={heroStyles.darkCardTitleStyle}>{card.title}</h3>
                </Box>
              </Link>
            </Grid>
          ))}
